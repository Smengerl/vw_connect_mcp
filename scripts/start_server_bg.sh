...
#   e.g. ./scripts/start_server_bg.sh "" --backend carconnectivity
# Defaults: config.json -> unset (tibber backend needs none); backend -> tibber (server default)
...
# extra mcp_server_cli flags, e.g. --backend carconnectivity.
if [ "$#" -gt 0 ]; then
  shift
fi

echo "Starting server (background) with config=${CONFIG:-<none, tibber backend>} extra_args=$*"
