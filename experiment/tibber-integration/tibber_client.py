"""Minimal Tibber Data API client — OAuth2 (Authorization Code + PKCE) + REST.

This is deliberately dependency-light (httpx + stdlib only) and structured so
the auth/token/API pieces can later be lifted into the MCP server's adapter
layer. It focuses on the *login flow* and read-only device access.

Reference: ../tibber-integration/TIBBER_API.md (endpoints, scopes, data model)
Modelled on evcc's implementation: https://github.com/evcc-io/evcc/pull/30487

SECURITY: This module never logs client secrets or tokens, and persists tokens
only to a local JSON file that is gitignored. Do not commit that file.
"""
...
# Minimal scope set to read vehicles. Add e.g. "data-api-chargers-read" later.
DEFAULT_SCOPES = [
    "openid",
    "profile",
    "email",
...
# The Data API requires a descriptive User-Agent on every request.
USER_AGENT = "weconnect-mcp-tibber-experiment/0.1.0 (github.com/weconnect_mvp)"
...
class TokenStore:
    """Persists a TokenSet to a local JSON file (gitignored).

    Written with 0600 permissions so tokens are not world-readable.
    """
    ...
    def save(self, tokens: TokenSet) -> None:
        # Write then chmod to keep the secret file private to the user.
...
class TibberDataAPI:
    ...
    Typical use:
        api = TibberDataAPI(client_id, client_secret, redirect_uri, store)
        api.ensure_authorized()   # runs browser flow once, then refreshes
        api.homes(); api.vehicles()
    """
    ...
    def ensure_authorized(self) -> None:
        """Guarantee a valid access token, running the browser flow if needed."""
    ...
    def _refresh(self) -> None:
        ...
        if not self.tokens.refresh_token:
            # No refresh token (missing offline_access?) — re-run browser flow.
            self.tokens = self._interactive_login()
            self.store.save(self.tokens)
            return
        data = {
        ...
        for home in self.homes():
            for dev in self.devices(home["id"]):
                ...
                if dev_id and dev_id not in seen:
                    seen.add(dev_id)
                    # Annotate with the owning home so callers can fetch detail.
