...
_get_tibber_charging_state()/_get_tibber_range_km() (tibber_adapter.py) and
vin_from_external_id() (tibber_client.py) are the genuinely Tibber-specific
production logic in this project -- unlike TibberAdapter itself (exercised
indirectly through the tools/ tests via TestAdapter), none of it had any
direct test coverage before. All of it is pure functions with no I/O, so
it's testable with fixture data taken directly from ARCHITECTURE.md §3.1
(the real, confirmed device-detail response shape) -- no network, no auth,
no mock adapter needed.
"""

from weconnect_mcp.adapter.tibber_adapter import (
    _get_tibber_charging_state,
    _get_tibber_range_km,
)
...
def test_charging_state_full_response():
    charging = _get_tibber_charging_state(DEVICE_DETAIL)
...
def test_charging_state_no_capabilities_returns_none():
    charging = _get_tibber_charging_state({"capabilities": []})
    assert charging is None


def test_range_km_converts_meters():
    """range.remaining is reported in meters; must be converted to km."""
    range_km = _get_tibber_range_km(DEVICE_DETAIL)
    assert range_km == 356.0


def test_range_km_missing_capability_returns_none():
    range_km = _get_tibber_range_km({"capabilities": []})
...
def test_range_km_non_meter_unit_passthrough():
    ...
    assert _get_tibber_range_km(detail) == 250.0
