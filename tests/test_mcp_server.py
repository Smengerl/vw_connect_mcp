...
logger.setLevel(logging.WARNING)  # Set default log level to WARNING for tests
...
async def test_mcp_resources_provide_valid_data(mcp_server: Any):
    ...
    for vehicle_id in vehicles:
        ...
        assert isinstance(vehicle_str, MockVehicle), f"vehicle_str for {vehicle_id} should be a MockVehicle"
...
async def test_mcp_get_vehicle_state(mcp_client, vehicle):
    ...
    logger.debug(f"Deserialized JSON from client: {returned_vehicle}")
    
    assert returned_vehicle["vin"] == vehicle.vin, f"VIN mismatch: expected {vehicle.vin}, got {returned_vehicle['vin']}"
    assert returned_vehicle["model"] == vehicle.model, f"Model mismatch: expected {vehicle.model}, got {returned_vehicle['model']}"
    assert returned_vehicle["manufacturer"] == vehicle.manufacturer, f"Manufacturer mismatch: expected {vehicle.manufacturer}, got {returned_vehicle['manufacturer']}"
    assert returned_vehicle["state"] == vehicle.state, f"State mismatch: expected {vehicle.state}, got {returned_vehicle['state']}"
    assert returned_vehicle["type"] == vehicle.type, f"Type mismatch: expected {vehicle.type}, got {returned_vehicle['type']}"
    assert returned_vehicle["odometer"] == vehicle.odometer, f"Odometer mismatch: expected {vehicle.odometer}, got {returned_vehicle['odometer']}"
    assert returned_vehicle["position"] == vehicle.position, f"Position mismatch: expected {vehicle.position}, got {returned_vehicle['position']}"
    assert returned_vehicle["capabilities"] == vehicle.capabilities, f"Capabilities mismatch: expected {vehicle.capabilities}, got {returned_vehicle['capabilities']}"